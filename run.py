    ...
    for model_index in range(num_models):
        if model_index == 0:
            ...
            model = WhisperASRModel(
                model_name="alvanlii/whisper-small-cantonese", device=device
            )
        elif model_index == 2:
            model = WhisperASRModel(
                model_name="Scrya/whisper-large-v2-cantonese", device=device
            )
        elif model_index == 3:
        ...
        if os.path.exists(f"results/{model_name}/{dataset_name}.json"):
            print(
                f"Results for {model_name} on {dataset_name} already exist. Skipping..."
            )
            continue
        ...
        with open(
            f"results/{model_name}/{dataset_name}.json", "w", encoding="utf-8"
        ) as f:
